...
{
    int 魔方LayerID;//魔方layerID
    public GameObject 被点到的小正方体;//当前点击的魔方
    Vector3 鼠标点到的位置;//鼠标点中位置
    Vector3 鼠标移动的方向;//鼠标移动方向
    Vector3 鼠标点击位置的法线;//鼠标点击位置法线
    Axis 魔方旋转的轴向 = Axis.None;//魔方旋转轴向
    bool 是否是顺时针转动;//是否旋转
    bool 是否正在旋转 = false;
    小方块[] 记录所有小方块数组;//魔方数组
    GameObject 旋转的点;//旋转点
    ...
    private void 检测鼠标拖动()
{
    if (Input.GetMouseButtonDown(0))
    {
        ...
        // 从刚才点击屏幕的射线的初始点，发射一条跟刚才射线方向一样的射线，并返回射线目标物体的属性
...
private void 开始旋转魔方(Vector3 叉乘结果)
{
    ...
    private void FixedUpdate()
    {
        if (是否正在旋转)
        {
            ...
            // 当当前旋转与目标旋转角度差很小时，直接设置到目标旋转，避免残留误差
