...
public class StencilFaceWatcherOrth : MonoBehaviour
{
   [System.Serializable]
    ...
    void Update()
    {
        ...
        Vector3 viewDirection = -referenceCamera.transform.forward.normalized;
        ...
        foreach (var face in faces)
        {
            ...
            float dotValue = Vector3.Dot(normal, viewDirection);
            
            // 调试信息：显示每个面的点积值
            if (Time.frameCount % 60 == 0) // 每60帧打印一次，避免刷屏
            {
                Debug.Log($"[StencilFaceWatcherOrth] 面 '{face.name}': dotValue={dotValue:F3}, threshold={face.visibleThreshold}, normal={normal}, viewDir={viewDirection}, direction={face.normalDirection}");
            }
